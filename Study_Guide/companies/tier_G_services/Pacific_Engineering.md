# Pacific Engineering
**Role(s):** Unknown — no confirmed India SWE employer  
**Tier:** G  
**Status:** insufficient

> 🚪 [Start](../../START_HERE.md) · 🗺️ [Map](../../README.md) · 🏢 [Companies](../README.md) · 📚 [Library](../../library/README.md) · 🔍 [Find](../../FIND.md)

## Process Overview
**Disambiguation failure.** Hits resolve to civil/structural firms (e.g. Pacific Engineers Kolkata; Pacific Engineering & Design PLLC; Pacific Engineering Technologies) — none match a documented large India SWE campus pipeline. No SWE interview experiences found for a confirmed tech “Pacific Engineering.”

## Online Assessment (OA)
Unknown.

## Interview Rounds
Unknown.

## Topic Weights
| DSA | OS | C/C++ | DBMS | CN | LLD | HLD | Concurrency | Aptitude |
|---|---|---|---|---|---|---|---|---|
| Unknown | Unknown | Unknown | Unknown | Unknown | Unknown | Unknown | Unknown | Unknown |

## Recurring Patterns & Tips
- Request exact legal name / careers URL before any further pass.

## Related topics
- [DSA Overview](../../topics/DSA/00_DSA_Overview.md)

## Resource Panel
- [Complete DSA Roadmap](https://drive.google.com/file/d/10e8Q1yVymwxWhz9LgcWxwrkpW9eMxRla/view)
- [30 Coding Patterns](https://drive.google.com/file/d/1EQHkfIdslaNfyfD0Py6hoNl8ZlXuEcYD/view)
- [OOPS Boss Sheet](https://drive.google.com/file/d/1s0HeZt05eck7cuwXxQZtDiZ_NEPfZwo6/view)
- [DBMS Boss Sheet](https://drive.google.com/file/d/1r1Yn5xhka9jpb75z8PO2SgD4g4F_Tcc_/view)
- [OS Boss Sheet](https://drive.google.com/file/d/1YquLAtpYmeTLMpyUfb6qd2rVwnQpsz74/view)
- [cppreference](https://en.cppreference.com/w/)
- See also: [../../00_INDEX.md](../../00_INDEX.md)

## Sources
_No attributable SWE process sources._

## Search log
| Channel | Result |
|---|---|
| GFG / LC Discuss / Glassdoor | Name-collision civil/structural firms only |
| Outcome | Disambiguation blocked |
