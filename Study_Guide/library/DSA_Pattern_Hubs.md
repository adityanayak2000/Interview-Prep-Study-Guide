# 🧮 DSA pattern hubs
_External pattern maps + a few cheatsheets. Do not copy LC lists into this repo._

## Why
Pick a pattern map, then drill via [DSA Overview](../topics/DSA/00_DSA_Overview.md) topic files (Arrays, Trees, Graphs, DP, …).

## Links

### Pattern maps (external)
- [Sean Prashad — LeetCode patterns](https://seanprashad.com/leetcode-patterns/)
- [AlgoMonster flowchart](https://algo.monster/flowchart)
- [AlgoMaster DSA patterns](https://algomaster.io/practice/dsa-patterns)
- [LearnYard practice](https://learnyard.com/practice/dsa/)
- [Awesome LeetCode Resources](https://github.com/ashishps1/awesome-leetcode-resources)
- [system-design-primer](https://github.com/donnemartin/system-design-primer)

### Core PDFs
- [Complete DSA Roadmap (98 Pages)](https://drive.google.com/file/d/10e8Q1yVymwxWhz9LgcWxwrkpW9eMxRla/view)
- [30 Coding Patterns](https://drive.google.com/file/d/1EQHkfIdslaNfyfD0Py6hoNl8ZlXuEcYD/view)
- [Linked List Cheatsheet](https://drive.google.com/file/d/1lDSq_JKCrY0L-j81UGmMI1nAqCU_cSJx/view)
- [DP Cheatsheet](https://drive.google.com/file/d/1fc0H-9Uhb-dJeIKBHsovWMlGWZ3gM3Fv/view)
- [Graph Cheatsheet](https://drive.google.com/file/d/1Oyu009O6X5wKw-ETC-GxsFDjHJTWDNXS/view)

### In-repo
- [STL Quick Reference](../topics/DSA/STL/00_STL_Quick_Reference.md)
- [DSA Overview — pattern hubs section](../topics/DSA/00_DSA_Overview.md)

## Which topic file to study next

Start: [DSA Overview](../topics/DSA/00_DSA_Overview.md) → then Arrays / Trees / Graphs / DP as needed for your track.

## See also

- [📚 Library portal](README.md) · [00_INDEX](../00_INDEX.md)
